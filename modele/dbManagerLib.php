...
function sqlDbManageConfig($db)
{
    ...
    sqlQuery($db, "
        ...
        CREATE TABLE IF NOT EXISTS $tableConfigTableName(
            ...
            Is_join_table TINYINT(1) NOT NULL DEFAULT 0
        )ENGINE = INNODB;
    ...
    if(!sqlIndb($db, [$tableConfigTableName, $columnsConfigTableName, $joiningDataTableName, $pivotDatasTableName]))
    //[shit-flag]
    {
        sqlQuery($db, "
    ...
    foreach($dbTables as $dbTable) 
    {
        ...
        foreach($joiningInfos as $constraintName => $joiningInfo) 
            if(
                ...
                !in_array($dbTable, sqlGetPivotJoinTables($db))
            )
            {   
        ...
        if(isset($_GET['restructure'])) sqlManageForeignKeysDatas($db, $dbTable, true);
    }
...
function sqlManageForeignKeysDatas($db, $tableName, $updatePivotTable = false, $userGrants = [], $columnsAttributes = [], &$pivotPrimaryValues = [], &$selectArray = [], &$showKeyJoinings, $tableRename = '')
{
    ...
    //var_dump("sqlManageForeignKeysDatas() begins for $tableName: " . (string)(microtime(true) - $startTime));
    ...
    "SELECT originalTable_.$showKeyColumnName AS rootTable,
    ...
    //var_dump("sqlManageForeignKeysDatas() ends for $tableName: " . (string)(microtime(true) - $startTime));
        ...
        foreach($elementsProperties as $elementProperties) if($elementProperties)
        {
            ...
            switch($clauseType)
            {
                ...
                case 'NATURAL JOIN':
                    ...
                    foreach((array)$elementProperties['rootTable']['joinKey'] as $nIndex => $rootJoinColumn)
                    {
                        ...
                        $result .= $elementProperties['rootTable']['Name'] . ".`$rootJoinColumn` = $extraTableName." . $elementProperties['extraTable']['joinKey'][$nIndex] . ' ';
...
function htmlFormatMysqlUserPriv($db, $userPrimaryValue, $indexUrl = 'index.php')
{
    ...
    $pivotTables = sqlGetPivotJoinTables($db);
...
function sqlGetPivotJoinTables($db)
{
    global $tableConfigTableName, $showKeyColumnName;
    return array_column(sqlSelect($db, "SELECT $showKeyColumnName FROM $tableConfigTableName WHERE Is_full_pivot_join_table = 1"), $showKeyColumnName);
}

function sqlGetFullPivotJoinKeys($db, $joinTable)   //CHANGE MOI CETTE MERDE
{
    global $tablesProperties;
    $foreignPrimaryKeys = [];
    if($tablesProperties[$joinTable]['Is_full_pivot_join_table'])
    {
        $columns = sqlGetColumns($db, $joinTable, false);
        foreach($columns as $column)
        {
            //Pour chaque colonne d'une table
            if(inString($column['Field'], 'ID_'))
            {
                //Si la colonne contient 'ID_' (=> clé primaire étrangère)
                $foreignPrimaryKeys[] = $column['Field'];
            }
        }
        return $foreignPrimaryKeys;
    }
    else
    {
        return 0;
...
function sqlGetExtraTablesAndPrimaryKeys($db, $table)
{
    $extraTablesAndPrimaryKeys= [];
    foreach(sqlGetJoinList($db, $table) as $constraintName => $joinData)
        if(array_intersect($joinData['rootTable']['joinKey'], $joinData['rootTable']['primaryKey']) === (array)$joinData['rootTable']['joinKey'])
        {
            ...
            $extraTablesAndPrimaryKeys[$joinData['extraTable']['Name']] = $joinData['extraTable']['joinKey'];
        }

    return $extraTablesAndPrimaryKeys;
}
...
function sqlGetSupposedJoins($db)
{
    ...
    $supposedJoins = [];
    $joinTables = sqlGetPivotJoinTables($db);
    foreach($joinTables as $table)
    {
        //Pour chaque table
        $foreignPrimaryKeys = $extraTablesDatas = [];
        $extraTablesAndPrimaryKeys = sqlGetExtraTablesAndPrimaryKeys($db, $table);
        
        
        foreach($extraTablesAndPrimaryKeys as $extraTableName => $extraTablePrimaryKey)
        {
...
function sqlJoin($db,   $rootTable,     //Table principale à laquelle on souhaite en joindre d'autres
                        ...
                        $selectArray,   //Afin de connaître les renames à mettre en clé de columnsAttibutes
                        ...
                        $datalistQuery = [])    
{
    ...
    foreach($joinArray as $joinElement) /*if(isset($joinElement['Table'], $joinElement['rootKey'], $joinElement['showKey']))*/
    {
        ...
        $rootKey = implode(',', $joinElement['rootTable']['joinKey']);
        $columnsAttributes[$rootKey] = 
        [
            'edittable' => $rootTable,
            'primarykey' => $rootPrimaryKey,
            'editkey' => implode(',', (array)$joinElement['rootTable']['joinKey']),
        ...
        sqlCheckAttributesToMakeColumnEditable($columnsAttributes[$rootKey]);
...
function sqlGetJoinList($db, $tableName, $full = false, $tableRename = '', $returnFilters = [])
{
...
)
{
    ...
    elseif($returnType === 'htmlDatalist')
    {
        ...
        if(is_string($showKeyConcat) AND inString($showKeyConcat, 'CONCAT('))
        array_unshift(
            $selectArray,
            [
    ...
    elseif($returnType === 'htmlTable')
    {
        ...
        $cellMisc = $originTable === $usersTableName
            ?   [array_column(sqlGetShowKey($db, $originTable), 'Name')[0] => "<a href = '$indexUrlWithGets&amp;uid=$idString'>$contentString</a>"]
            :   NULL;
...
function htmlFilterBox($get, $keys, $indexUrl, $selectArray)
{
    ...
    "<form method = 'GET' action = '$indexUrl' id = 'filter-box'>" . 
        ...
        "<select name = 'f" . $lastFilter . "_Operator'>
            ...
            <option value = '<'><</option>
            <option value = '>'>></option>
            <option value = 'REGEXP'>comprend</option>
        </select>" .
